...
<%
	...
	<div class="row">
			...
			String query = "select * from bus where id = ?";
				...
				while(rs.next()){
			%>
				<h1 class="text-center">Edit Bus Details</h1>
				<input type="hidden" name="id" value='<%=rs.getInt("id") %>'/>				
				<div class="form-group">
					<label class="control-table" for="capacity">Capacity</label>
					<input type="number" name="capacity" id="capacity" class="form-control" value='<%=rs.getInt("capacity") %>'>
			...
			<div class="form-group">
				...
				<select id="condition" name="condition" class="form-control" value='<%=rs.getString("bus_condition") %>'>
					<option>New</option>
					 <option>Good</option>
					 <option>Fair</option>
					 <option>Bad</option>
			...
			<input type="submit" class="btn btn-warning">Update</input>
...
<jsp:include page="dashboarddown.jsp">
